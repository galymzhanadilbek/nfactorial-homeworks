...
class Flight:
    ...
    def delay(self, delay_time):
        self.departure = str(int(self.departure.split(':')[0]) + delay_time) + ":" + self.departure.split(':')[1]
...
class Matrix:
    ...
    def multiply(self, other):
        ...
        result = [
            [sum(self.matrix[i][k] * other.matrix[k][j] for k in range(len(self.matrix[0]))) for j in
             range(len(other.matrix[0]))]
            for i in range(len(self.matrix))
        ]
        return Matrix(result)
